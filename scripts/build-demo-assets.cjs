/* Regenerates the hero fallback illustration shown before (or without) the 3D model. */
const fs = require('fs');
const path = require('path');
const assets = path.join(__dirname, '..', 'assets');
const variants = [
  ['natural', '#e5dac1', '#faf4e5', '#b8603a', '#5c5b4b', 'REPACK', 'made for', 'what’s inside.', 'SPOUT BAG']
];
for (const [name, base, light, cap, ink, brand, line1, line2, note] of variants) {
